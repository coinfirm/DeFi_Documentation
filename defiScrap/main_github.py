...
from defiScrap.spiders.gitSpider import GitspiderSpider
import pandas as pd 
import re
...
owners = github_frame['Domena'].to_list()
links = github_frame['Link do dokumentacji'].to_list()
process = CrawlerProcess()

for i in range(0, len(links)):
    o_name = links[i].replace('https://github.com/', '')
    if o_name.find("/") != -1:
        o_name = o_name.split('/')[0]
    filename = owners[i].replace('.', '_')
    filename = filename.replace('/', '_')
    process.crawl(GitspiderSpider, start_urls = [str(links[i])], allowed_domains = ["github.com"], filename = filename, ownername = o_name)
...
for i in range(0, len(links)):
    ...
    if os.path.exists(paths['paths'][mode]['working_dir'] + f"/gitaddresses_{filename}.csv"):
        current_data = pd.read_csv(paths['paths'][mode]['working_dir'] + f"/gitaddresses_{filename}.csv", names= columns, header=None)
        ...
        current_data.to_csv(paths['paths'][mode]['working_dir'] + f"/gitaddresses_{filename}.csv")
    else:
        print(f"couldn't find file {paths['paths'][mode]['working_dir']}/gitaddresses_{filename}.csv")

evi = Evidence('test')
for i in range(0, len(links)):
    filename = owners[i].replace('.', '_')
    filename = filename.replace('/', '_')
    current_data = pd.read_csv(paths['paths'][mode]['working_dir'] + f"/gitaddresses_{filename}.csv")
...
for i in range(0, len(owners)):
  ...
  if not os.path.exists(os.path.join(paths['paths'][mode]['working_dir'], f"gitaddresses_{filename}.csv")):
    print(f"Can't find file for owner {filename}")
  else:
    current_data = pd.read_csv(os.path.join(paths['paths'][mode]['working_dir'], f"gitaddresses_{filename}.csv"))
    ...
    for i in range(0, len(addresses)):
        ...
        if is_weird:
            ...
            requests.post(paths['credentials']['slack']['url'], json={"text": f"Address `{addresses[i]}` for owner {ow_id} looks weird :face_with_raised_eyebrow:, please check"}, headers={"Content-Type": "application/json"})  
        else:
